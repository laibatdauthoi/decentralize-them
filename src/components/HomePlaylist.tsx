...
const MODULE_ADDRESS1 = process.env.NEXT_PUBLIC_MODULE_ADDRESS;
...
export default function HomePlaylist() {
  const { playTrack, formatTitle1, currentTrack, isPlaying, arenaStats, refreshArenaStats } = useMusic();
  ...
  const [isSuccess1, set_is_success] = useState<boolean>(false);

  useEffect(() => {
    if (arenaStats) set_tracks_list(arenaStats);
  }, [arenaStats]);
  ...
  const isOwner = (artistAddr: any) => {
    ...
    const normalize = (addr: { toString: () => string }) => addr.toString().toLowerCase().replace(/^0x0*/, "0x");
  ...
  const handleConfirmLove1 = async (track: any) => {
    ...
    try {
      const payload1: InputTransactionData = {
        data: {
          function: `${MODULE_ADDRESS1}::${MODULE_NAME1}::love_song` as `${string}::${string}::${string}`,
          typeArguments: [],
          functionArguments: [String(track.blobName), Math.floor(AmountNum1 * 100_000_000).toString()],
        },
      };

      await signAndSubmitTransaction(payload1);
      ...
      console.error("Love transaction error:", e);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-0 md:px-0 min-w-0">
      <div className="w-full py-6 text-center">
        <h1 className="text-lg sm:text-xl md:text-2xl font-black uppercase tracking-[0.16em] sm:tracking-[0.25em] text-white">
      ...
      {tracksList.length > 0 && (
        <div className="flex justify-center -mt-2 mb-6">
          <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-3 sm:px-4 py-1.5 rounded-xl backdrop-blur-sm max-w-full">
          ...
          return (
            <div key={track.blobName}>
              <div
                ...
                style={{ minHeight: "76px", padding: "0 clamp(12px, 2vw, 20px)", borderRadius: "4rem" }}
              >
                ...
                <div className="flex items-center gap-2 md:gap-4 shrink-0 ml-2">
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 md:px-5 py-2.5 rounded-full border transition-all duration-300 cursor-pointer ${isLovingThis ? "opacity-50 cursor-wait" : "hover:scale-110 active:scale-95"} ${isActive ? "bg-[var(--pink_main)] border-[var(--pink_main)] text-white shadow-[0_0_20px_rgba(236,72,153,0.5)] animate-pulse" : "bg-white/5 border-white/10 text-white/40 hover:text-white hover:border-[var(--pink_main)] hover:bg-[var(--pink_main)]/20"}`}
                  >
                    {isLovingThis ? (
                      <Loader2 size={16} className="animate-spin" />
                    ) : (
                      <>
                        <span className="text-[10px] font-black uppercase tracking-[0.1em] hidden md:block">Love it</span>
                        <Heart size={16} className={isActive ? "fill-white" : ""} />
                      </>
                    )}
                  </button>
                </div>
              ...
              {isSelectingThis && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                  <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => !isSuccess1 && set_selected_push_id(null)} />

                  <div className={`relative bg-[#0d0d0d] border ${ErrorMsg1 ? "border-red-500" : "border-[var(--pink_main)]/40"} rounded-[2.5rem] p-6 sm:p-8 w-full max-w-xs max-h-[calc(100dvh-2rem)] overflow-y-auto shadow-[0_0_50px_rgba(236,72,153,0.2)] animate-in zoom-in-95 duration-200`}>
                    <button onClick={() => set_selected_push_id(null)} className="absolute top-6 right-6 text-white/20 hover:text-white cursor-pointer">
                      <X size={20} />
                    </button>
                    ...
                    <div className="flex flex-col items-center gap-6">
                      ...
                      <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col items-center gap-1 focus-within:border-[var(--pink_main)]/50 transition-colors">
                        ...
                        <div className="flex items-center justify-center gap-3">
                          <button
                            ...
                            className="bg-white/5 hover:bg-[var(--pink_main)]/20 text-white rounded-full p-3 transition-colors active:scale-95 cursor-pointer"
                          >
                            <Minus size={20} />
                          </button>
                      ...
                      <button
                        ...
                        disabled={isLovingThis || userIsOwner || isSuccess1}
                        className={`w-full text-white py-4 rounded-full font-black uppercase text-[12px] flex items-center justify-center gap-3 transition-all ${isSuccess1 ? "bg-green-500 shadow-[0_0_25px_rgba(34,197,94,0.4)]" : userIsOwner ? "bg-red-500/20 text-red-500 cursor-not-allowed opacity-50" : "bg-[var(--pink_main)] hover:shadow-[0_0_25px_rgba(236,72,153,0.5)] active:scale-95 cursor-pointer"}`}
                      >
                        {isLovingThis ? (
                          <Loader2 size={16} className="animate-spin" />
                        ) : isSuccess1 ? (
