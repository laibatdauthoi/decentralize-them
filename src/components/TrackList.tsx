...
export function TrackList({
  ...
  const {
    ...
    playTrack,
    refreshArenaStats,
    pushSong1, // Lấy hàm mới từ context
  } = useMusic();
  ...
  // --- CẬP NHẬT HÀM PUSH THEO CONTRACT V5 ---
  const handleConfirmPush1 = async (Item1: track_info) => {
    ...
    showStatus1(Item1.id, "PREPARING TRANSACTION...", "info");

    try {
      // Đổi đơn vị sang Octas (Aptos dùng 10^8)
      const OctasAmount1 = Math.floor(AmountNum1 * 100_000_000);

      // Gọi trực tiếp hàm đã khai báo trong MusicContext
      await pushSong1(signAndSubmitTransaction, Item1.blobName, OctasAmount1);
  ...
  if (isLoading) return <div className="text-center py-16 animate-pulse text-[var(--pink_main)] font-bold tracking-widest text-sm md:text-base uppercase">Syncing Blobs...</div>;
  if (error) return <div className="text-center py-16 text-red-400 bg-red-400/10 rounded-[3rem] border border-red-400/20 mx-4"><AlertCircle className="mx-auto mb-2" size={20} />SYNC ERROR</div>;

  return (
    <div className="flex flex-col gap-2 w-full relative z-0 min-w-0">
        ...
        return (
          <div key={Item1.id}>
            <div
              ...
              `}
              style={{ minHeight: "86px", padding: "0 clamp(12px, 2vw, 20px)", borderRadius: "4rem" }}
            >
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  {MyStatus1 ? (
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 mt-0.5 min-w-0">
                      ...
                      <span className={`text-[9px] font-black flex items-center gap-1 uppercase min-w-0 ${IsActive1 ? "text-[var(--pink_main)] opacity-80" : "opacity-30 text-white"}`}><Wallet size={11} />{formatAddress1(Item1.walletAddress)}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {IsSelectingThis1 && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <div className="relative bg-[#0d0d0d] border border-[var(--pink_main)]/40 rounded-[2.5rem] p-6 sm:p-8 w-full max-w-xs max-h-[calc(100dvh-2rem)] overflow-y-auto shadow-[0_0_50px_rgba(236,72,153,0.2)] animate-in zoom-in-95 duration-200">
